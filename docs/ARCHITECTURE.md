...
`ml/calibration.py::fit_calibration` fits a `HuberRegressor` (robust to lag-artefact outliers) mapping IBJA's per-gram rate to Tanishq's 22K retail price — a scale/offset calibration, not a full premium model, requiring ≥30 overlapping days before it's trusted (currently valid: R²=0.96, n=51+). Auto-refits every 10 new pairs. Per **ADR 025** (inverting ADR 021's original fallback framing now that Tanishq's block is sustained rather than occasional), this calibrated estimate is the **primary** displayed price whenever the Tanishq scrape isn't fresher than 8h; a fresh scrape is an enrichment that overrides it with a directly-confirmed reading. The display gate itself is a generous 14-calendar-day backstop (`_IBJA_DISPLAY_MAX_AGE_DAYS`) — deliberately loose since IBJA publishes ~5x/week and genuinely showing something honestly dated beats falling back early. Separately, `ml/ibja.py::compute_ibja_gap_business_days` measures IBJA's own freshness in **business days** (weekends never count — IBJA doesn't publish Sat/Sun/govt holidays) to drive the T9/T9_ESCALATE alerts in `ml/notifications.py`, so a normal weekend carry-forward never false-alarms but a genuine multi-day IBJA outage does. `ml/ibja.py` fetches daily AM/PM rates from ibjarates.com each cycle and separately supports a monthly 30-day PDF backfill (`monthly-ibja-backfill.yml`).
...
Progressive Web App served from GitHub Pages, scoped by `_config.yml` to just the app surface (see the repo/deploy cleanup, 2026-07-18). Reads `data/*.json` directly. Two independent freshness guards: a UI-facing stale banner + freshness pill (`app.js::renderStaleBanner`/`renderFreshness`) that trust `forecast.price_source` — honestly labeling "estimated retail (calibrated from IBJA)" when that's the primary path (with an explicit "as of {weekday} close" qualifier on weekend/holiday carry-forward), and only falling to the genuine "may be outdated" copy when IBJA itself is beyond its display backstop — and a CI-facing `scraper-canary.yml` (weekly + on scraper-path PRs) that runs a live scrape against the real Tanishq page to catch DOM/selector breakage before it reaches production (independent of whether Tanishq is currently reachable in production).
...
<!-- scratch test line, Z1c labeled-leak repro, will be reverted -->
