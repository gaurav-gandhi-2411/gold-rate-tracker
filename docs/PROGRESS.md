...
- **ntfy topic rotation cancelled (WONTFIX, 2026-06-07):** GG accepts `gold-***` (value redacted 2026-06-14) as the live topic. Residual risk: public ntfy topic permits unsolicited publishes (spam) only; no data, repo, or pipeline access. Revisit only if product goes multi-tenant. CURRENT_STATE.md item (d) updated accordingly. **SUPERSEDED 2026-06-14:** the literal topic had been committed here and in CURRENT_STATE.md (so it is exposed in git history) — value now redacted; rotation is now an owner action. See CURRENT_STATE.md item (d).
