...
| 2026-06-01 | PR Φ5 — calibration refit wired; fit triggers at ≥30 pairs (initial unlock) or ≥10 new pairs (periodic refit) | CC (ADR 017) | run_refit_if_needed() added to calibration.py, called from CI after IBJA append. Probe commit consolidated into main data commit (fixes staleness race). drift.py writes baseline_mae per entry. Post-merge finding: 8 live-append rows (2026-05-19 to 05-29) all have pm_916=NaN (CI runs at 10:30/16:30 IST, before ~17:00 IST PM fix). Effective valid overlap = 21, not 29. Overlap counter fixed to filter pm_916 non-null. Backtest: 185-row parquet still yields 165 folds / same MAE (177 non-null rows effectively). ADR 012 confirmed at larger data volume. Calibration unlock ETA: ~9 more valid pairs (~2026-06-12). |
...
| 2026-06-02 | Production cron 4h → 3h; stale copy corrected (WI-1+WI-3, PR #57/Φ6A) | CC | WI-1: 3h = 8 runs/day; T8 morning/evening windows each still contain ≥1 slot; calibration unlock ETA (~2026-06-12) unchanged (IBJA-daily-gated). WI-3 diagnosis: "every 3 days" was static copy in app.js renderModelSignal(), not a data throttle — forecast.json updates every CI cycle; T7's 3-day floor is a separate notification mechanism unrelated to display. Footer "6 hours" also corrected. Pre-existing ruff F841 + import ordering in test_calibration_unlock_chain.py fixed as blocking blocker on all branches. |
| 2026-06-02 | Commentary SYSTEM_PROMPT refined for warmer plain-language tone (WI-2, PR #58/Φ6B) | CC | Source confirmed: ml/commentary.py produces the "Today's read" section via Groq LLM. Tone improvements: "brief, friendly notes for Indian families" replaces "plain, factual notes"; 80→90 word limit; 2-3→2-4 sentences; "warm everyday English"; directional framing uses "gently note which way prices may lean." All 8 jargon-blocklist terms retained in NEVER USE list; test_system_prompt_blocks_technical_jargon green; honest-baseline framing preserved. |
...
| 2026-06-03 | Φ16: honesty-state closes — refresh-fail visible, offline indication, load-error user copy, number tick, initial stagger | CC + GG (approved) | Five items closing gaps surfaced by PART 1 inventory. (1) Refresh-fail: `refreshData()` catch writes "Couldn't refresh — showing last update from {rel_time}" to `#stale-banner`; `renderStaleBanner()` always resets first (also fixes a pre-existing bug where a stale banner could survive a successful-but-fresh-forecast refresh). (2) Offline: `#offline-banner` element + `updateOfflineBanner()`; `offline`/`online` listeners registered before data load; offline suppresses stale-banner (redundant to stack). (3) Load-error copy: "Error loading data" → "Price unavailable"; dev copy → `console.warn`; user commentary → "Couldn't load the latest price. Check your connection and try again." (4) Number tick: `animateNumberTick()` RAF cubic ease-out 400ms; `matchMedia('prefers-reduced-motion: reduce')` JS gate (CSS global override won't suppress JS counters); first render instant. (5) Initial stagger: `staggerEnter()` added to `init()` after `renderDriverContext()` — was refresh-only before, making first render the one time it didn't animate in. §11 rows 11+12 added. SW v13→v14. Freshness thresholds kept 8h/18h (GG: 4h would cry wolf at 3h cron cadence with CF gaps). Hero sparkline left muted (GG: restraint = on-brand). |
| 2026-06-03 | Φ15: "say what we mean" pass — 11 label/grammar/card fixes, 8th copy-path miss, §11 static-label blind-spot closed | CC + GG (approved) | Three items. (1) ITEM 1 — 11 "forecast/predict/expected" label/header fixes across index.html + app.js (static HTML and template-literal strings): h2/aria/summary/canvas labels; "Predict no change"→"Assume no change"; "5-day expected range"→"5-day range"; "How accurate is this forecast?"→"How accurate is this?"; "Forecast accuracy—last 7d"→"Estimate accuracy—last 7d"; "Flat-hold forecast" chart legend→"Flat-hold estimate"; verdict-rule body "forecast or 30-day average"→"estimate or 30-day average"; flat-hold caption "forecast (dashed)"→"estimate (dashed)". KEPT: "Today's price is the forecast." + "beat simply using today's price as the forecast" — both honest self-aware body copy describing what flat-hold substitutes for, not a prediction claim. §11 BLIND-SPOT FIX: §11 only listed JS copy-generating functions; static HTML labels and hardcoded template-literal strings were outside its coverage — which is why this regression persisted. Added rows 9 (index.html static) + 10 (app.js template strings) to §11 + updated grep to include `grep -in "forecast\|predict\|expected" index.html`. This is the 8th copy-path miss and the first one whose root cause was a structural gap in the audit inventory, not just an audited-but-missed path. (2) ITEM 2 — grammar fix in flat verdict branch: "Prices are edged down ₹55" (ungrammatical present-simple + past-participle) → "Prices have edged down ₹55 this week — within the typical weekly range. No strong signal either way." Up/down branches were already grammatically correct ("have slipped"/"have risen"). "Within the typical weekly range" explicitly bridges the hero-verdict / LLM-commentary apparent contradiction. absSlope<20 branch ("Prices are virtually flat") was grammatically correct; unchanged. (3) ITEM 3 — "vs 30-day low" card label→"30-day floor"; "above 30d low" sub-text→"above this month's lowest". Reframe (not drop): floor-distance is a distinct buyer question from average-distance, now clearly labelled. SW VERSION v12→v13. |
...
| 2026-06-03 | Φ14-2: driver-context display — three-branch driver-state (PR-Φ14-2) | CC + GG (copy approved) | `renderDriverContext()` in app.js. Three 30d branches: B1 (driver >2%) — state + mechanism sentence; B2 (drivers <2%, premium >1%, current live state) — "Indian gold has moved more than global prices or the rupee explain this month — local factors such as import costs or seasonal demand are driving the difference." (humble-about-why load-bearing — 'such as' framing, do NOT assert specific cause); B3 (all flat) — "Gold has been stable this month; no major driver moved much." Driver threshold: 2% (GG: mechanism sentence only fires on clearly noticeable moves). Attribution headline (7d, valid-only) and driver-state both past-tense, no forecast, no buy/sell. PREMIUM-DOMINANCE NOTE (load-bearing for future work): premium stability CV 0.021 is a long-run average; the live 30d window ending 2026-06-02 shows premium expanded 5% while gold_usd was flat (domestic story, not a global one). Any future work relying on premium-stability must re-check the live window CV before assuming stability. Consumer audit (norm #15): commentary.py and notifications.py do not read driver_context — confirmed by grep; clean. SW VERSION bumped v11→v12. RUNBOOK §11 path #8 updated. index.html section, style.css, service-worker.js changed. |
...
| 2026-06-14 | ntfy topic NOT rotated despite git-history exposure (WONTFIX) | GG | Close-out: the live topic was found committed in CURRENT_STATE.md/PROGRESS.md/README (so it is in git history, effectively public). Value redacted to `gold-***` in the working tree (#131). Owner decided NOT to rotate: an exposed ntfy topic is publish-only — worst case is unsolicited notification spam to the owner's own device, with no data/repo/pipeline access. Exposure accepted; no action pending. Revisit only if multi-tenant or if spam becomes a nuisance (rotation is then a one-line `NTFY_TOPIC` secret change). Supersedes the interim "rotation required" note in CURRENT_STATE item (d). |

### Phi8A — Pipeline Hardening (2026-06-02)

**WI-Phi8A: Integration test + schema contracts**
...
### Batch Φ16 — App-feel: honesty-state + motion (2026-06-03)  ✅ COMPLETE

**Goal:** Close honesty-state gaps surfaced by PART 1 inventory; add motion that conveys state (not decoration). Guiding principle: HONEST RESTRAINT — every addition must help the user understand what's happening, not be flourish.

**PR-Φ16 (2026-06-03)** ✅ MERGED

- **Item 1 — Refresh-failure visible state (honesty):** `refreshData()` catch now shows "Couldn't refresh — showing last update from {rel_time}" in `#stale-banner`. Persists until next successful refresh. `renderStaleBanner()` refactored to always reset the banner first (incidental bug fix: previously a stale message could survive a successful refresh if forecast was fresh).
- **Item 2 — Offline indication (honesty):** `#offline-banner` element added. `updateOfflineBanner()` checks `navigator.onLine`; `offline`/`online` event listeners registered early in `init()` (before data load). Offline banner suppresses `#stale-banner` (precedence: "Offline" already explains staleness — stacking both is redundant and alarming). `online` handler hides offline banner and re-evaluates stale-banner using `lastForecast`.
- **Item 3 — Load-error copy (honesty):** Hero: "Error loading data" → "Price unavailable". Commentary: dev-facing "run the workflow from the Actions tab" string → "Couldn't load the latest price. Check your connection and try again." Dev hint moved to `console.warn`.
- **Item 4 — Number tick (motion that conveys state):** `animateNumberTick()` — RAF loop, cubic ease-out, 400ms. `matchMedia('(prefers-reduced-motion: reduce)')` gate explicit in JS (CSS global override doesn't suppress JS counters). Fires only when price changes on a live refresh and `priceEl` is already visible; first render and no-change are instant.
- **Item 5 — Initial-load stagger (consistency):** `staggerEnter()` now called after `renderDriverContext(fc)` in `init()`. Previously only called in `refreshData()` — first render was the one time cards didn't animate in, which was backwards.
- RUNBOOK §11 updated: rows 11 (error copy) and 12 (offline copy).
- SW VERSION bumped: v13→v14.

**Skipped (inventory confirmed already exists):** pull-to-refresh, chart fade, header tinting, freshness three-tier pill, prefers-reduced-motion global override. Freshness thresholds kept at 8h/18h (correct for 3h cron + CF gaps). Hero sparkline left muted (restraint = on-brand).

**Device-verify required (§10):** offline banner (airplane mode), refresh-fail state (network throttle), number tick (live refresh with price change), initial-load stagger — all with prefers-reduced-motion on AND off.
...
**PR-Φ14-2 — Display (2026-06-03)** 🟡 OPEN (awaiting CI green)

Copy approved: all three branch strings + headline patterns (GG 2026-06-03). Driver threshold 2% (GG: mechanism sentence at clearly noticeable moves). index.html section + app.js `renderDriverContext()` + style.css + SW VERSION v11→v12. Consumer audit clean. Post-deploy: GG visual confirm Branch 2 "local factors" string visible (current live state).
...
- **ntfy topic rotation cancelled (WONTFIX, 2026-06-07; reaffirmed 2026-06-14):** GG accepts `gold-***` (value redacted 2026-06-14) as the live topic. Residual risk: public ntfy topic permits unsolicited publishes (spam) only; no data, repo, or pipeline access. The literal topic was committed here / in CURRENT_STATE.md (so it is in git history); value now redacted, but the owner has **decided not to rotate** — exposure accepted (see Decision Log 2026-06-14). Revisit only if multi-tenant or if spam becomes a nuisance.
...
| 2026-08-10 | Retire `ml/commentary.py` (Groq LLM blurb) rather than repurpose it for driver-attribution narrative | Its only real consumer was `app.js`'s "Today's read", which moved to a deterministic client-side sentence (`composeTodaysRead()`) built from signals already computed for the good-price card — no LLM call, no restated numbers, no direction prediction. `notifications.py`'s `_check_t4` "consumer" was already dead code (`.get()` on a JSON list, `AttributeError` silently swallowed). Considered repurposing for a driver-attribution note instead (`driver_context` is already sitting in `forecast.json`, unread by commentary.py per the Φ14-2 consumer audit) — rejected: the pipeline has no external data source (no news/market API), so a genuinely value-adding narrative would have to be invented by the model rather than derived from data, which is exactly the hallucination risk this codebase's honesty guardrails (DARK gate, banned-words/content-policy checker, ADR-level rigor on "no directional edge") exist to prevent. A grounded version (just paraphrasing `driver_context`'s own numbers) would only restate what the driver-context section already states directly below it — the same anti-pattern "Today's read" was replaced for. | `ml/commentary.py` and `tests/test_commentary.py` deleted; `check-price.yml`'s commentary step and `GROQ_API_KEY` env removed; `data/commentary.json` left in place as a historical artifact (past entries aren't reproducible) but nothing produces or reads it going forward. `GROQ_API_KEY` repo secret itself needs manual deletion (Settings → Secrets) — not done here, deleting a live secret is outside what CC does autonomously. |

<!-- scratch test line, Z1c clean-PR baseline, will be reverted -->
