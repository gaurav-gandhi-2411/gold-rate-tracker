...
**Item 3 — [skip ci] suppressing master Lint runs (`lint.yml`).** Squash-merging PRs that contained `docs(progress)` commits with `[skip ci]` in the commit body propagated the annotation to the squash commit body. GitHub Actions then skipped ALL push-triggered workflows on that master push (confirmed on Ψ2B #40 and Ψ2C #41). Investigation result: GitHub's `[skip ci]` is applied platform-wide per-commit for `push` and `pull_request` events — there is no `on:` trigger configuration that allows a specific workflow to run while others are skipped. Option D as specified ("explicit Lint trigger on master push regardless of commit message content") is technically infeasible on GitHub Actions free tier. Fix implemented: `schedule: cron '0 6 * * *'` (06:00 UTC daily, 11:30 IST, between bot cron windows) and `workflow_dispatch` added to `lint.yml`. `schedule` triggers are not suppressible by commit message annotations — Lint always runs on master at least daily regardless of any `[skip ci]` in recent commits. Bot cron commits (chronos probe, gold prices) still correctly skip push-triggered Lint. Root cause convention: docs commits on PR branches must not carry `[skip ci]` (only standalone direct bot commits to master should use it). Residual gap: up to ~24h window after a [skip ci] squash merge before the daily backstop fires.
...
| 2026-05-30 | lint.yml backstop: daily schedule (06:00 UTC) + workflow_dispatch added (PR Ψ3A) | CC | Option D ("always run on master push regardless of [skip ci]") is technically infeasible — GitHub [skip ci] is platform-level per-commit for push events, no per-workflow override exists. Daily schedule is the closest achievable mechanism. Residual gap acknowledged: up to 24h window after a [skip ci] squash merge. Convention adopted: docs commits on PR branches must not carry [skip ci]. |
