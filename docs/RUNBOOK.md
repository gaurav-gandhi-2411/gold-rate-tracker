...
| 4 | `app.js` | `computeVerdict()` | Verdict card headline + reason: "Getting cheaper this week" / "Getting pricier this week" / "Steady this week", reason "Down/Up ₹X this week[, and ₹Y below/above the usual price for the month]." or "Barely moved this week — nothing to react to." / "Prices edged up/down ₹X this week — that's normal movement, nothing to react to." | Φ9A (INV-2) → verified 2026-08-10 (#765 rewrite) |
| 5 | `app.js` | `computeGoodPriceSignals()` | Good-price verdict lead (4-tier: "You're paying less than usual this month" / "a little less than usual this month" / "about the usual amount this month" / "a bit more than usual this month"), proof line — now a day-count, not a percentage ("Cheaper than N of the last M days." / "Pricier than N of the last M days."), data-sufficiency degrade note (unchanged: "Only N distinct days in the window — treat as indicative."), supporting lines ("Cheaper than most days this month." / "A bit below the usual price this month." / "Right around the middle for this month." / "Pricier than most days this month."; "₹X below/above the usual price for the month." / "Right at the usual price for the month."), divergence note ("These two don't quite agree — one counts days, the other measures the actual rupee gap. We go with the day-count for the headline above.") | Φ18A (2026-06-05) → verified 2026-08-10 (#765 rewrite) |
| 6 | `app.js` | `renderModelSignal()` vol-context block | 4 regime-conditional strings: "Gold has been more volatile than usual lately — about ±₹X over 5 days." / "Gold has been calmer than usual lately — about ±₹X over 5 days." / "Gold has been moving about ±₹X over 5 days lately." / "Gold's price typically moves about ±₹X over 5 days." | Φ10B → verified unchanged 2026-08-10 (#765 did not touch this surface) |
| 7 | `app.js` | `renderMethodology()` — Direction signal + "How accurate is this?" sections | Direction-signal meth-note: 'We test our price-direction models every week. So far, none of them beat just assuming "gold usually goes up" — so we don't show a chance-of-rising percentage or tell you to buy or sell. The trend labels above (Getting cheaper/pricier/Steady) describe what already happened this week — they're not a prediction of what happens next.' "How accurate is this?" panel: flat-hold framing ("We assume tomorrow's price is about the same as today's" + off-by-₹X-on-average narrative, negative result kept explicit — nothing beat flat-hold); range-coverage framing ("Our {range} range has been right X% of the time (checked N times so far)" / "...close to on target so far — still building a track record"); direction-signal recap ("Our AI was right X% of the time across N test windows. But gold rises on roughly 70% of trading days anyway...") — no-directional-edge claim kept explicit throughout, corrected function attribution (this lives in `renderMethodology()`, not `renderModelSignal()`) | Φ8C' → corrected + verified 2026-08-10 (#765 rewrite) |
| 8 | `app.js` | `renderDriverContext()` | Attribution headline (7d, only when attribution_valid=True, now ₹ not "Rs."): "Gold is up/down about ₹{total} this week — mostly a weaker/stronger rupee (₹{x}), plus/with a bit from global gold prices (₹{y})." (6 total phrasing variants by dominant driver). Driver-state (30d, 3-branch): B1 (driver >2%): mechanism sentences ("a weaker rupee makes imported gold pricier in India." etc.); B2 (premium-dominated, both drivers <2%): "Indian gold has moved more than the global price or rupee explain — likely import costs or festival demand at home."; B3 (all flat): "Nothing much moved this month — global prices, the rupee, and local demand have all been quiet." — PAST-TENSE ONLY, no forecast, no buy/sell (unchanged constraint). | Φ14-2 (2026-06-03) → verified 2026-08-10 (#765 rewrite) |
| 9 | `index.html` | Static section headings, `<summary>` accordion text, `aria-label` attributes | Section `<h2>` labels: "Is today a good time to buy?" (was "How does today's price compare?"), "How past estimates have held up" (unchanged), "What's moving the price?" (unchanged); track-record aria-label "Past estimate accuracy — flat-hold vs actual prices" (unchanged); methodology accordion summary "How this works — and how accurate it's been" (was "How this works · how good is this? · historical checks"); canvas aria-labels (unchanged); comparison card label "30-day floor" (unchanged) | Φ18C (2026-06-05) → verified 2026-08-10 (#765 rewrite) |
| 10 | `app.js` | Hardcoded strings inside `renderMethodology()` template literals and `renderComparisons()`/`renderForecastVsActual()` chart config | Methodology card heading/stat strings — corrected: the "Next trading day range" section heading (row was previously mislabeled "5-day range", which does not exist in `app.js`), "Assume no change" (unchanged), "How accurate is this?" (heading unchanged), "Estimate accuracy — last 7 days" (unchanged); Chart.js dataset label "Flat-hold estimate" (unchanged, lives in `renderForecastVsActual()` not `renderMethodology()`); verdict-rule body now "estimate or monthly average" (was "estimate or 30-day average"); floor card sub-text "above this month's lowest" (unchanged); dynamic accuracy clause in direction-signal meth-note (`bt.dir_acc_5d_chronos`, unchanged) | Φ18C (2026-06-05) → corrected + verified 2026-08-10 (#765 rewrite) |
| 11 | `app.js` | `refreshData()` catch block; `init()` load-error catch block | State-honesty error copy: "Couldn't refresh — this is the last update, from {rel_time}" (was "Couldn't refresh — showing last update from {rel_time}"; refresh fail, `#stale-banner`); "Price unavailable" (hero on load fail, unchanged); "Couldn't load the latest price. Check your connection and try again." (unchanged — shown in the `#commentary-text` element, which now holds the today's-read synthesis rather than Groq commentary, on load fail) | Φ16 (2026-06-03) → verified 2026-08-10 (#765 rewrite) |
| 12 | `app.js` | `updateOfflineBanner()` | Offline copy: "You're offline — showing prices from {rel_time}" and "You're offline — no prices loaded yet" (was "Offline · showing last loaded data from {rel_time}" / "Offline · no data loaded yet") — shown in `#offline-banner` when `navigator.onLine === false` | Φ16 (2026-06-03) → verified 2026-08-10 (#765 rewrite) |
| 13 | `app.js` | `composeTodaysRead()` | "Today's read" (`#commentary-text`, `.commentary-eyebrow`): a qualitative 1-sentence synthesis from `computeGoodPriceSignals()`'s tier + `computeTrendResidual30d()`'s trend state — e.g. "Today's price is on the low side for the month, and it's still sliding — it hasn't leveled off yet." (11 branches: no-signals fallback; no-trend-data × {cheap/high/mid}; cheap × {still-falling/steadying}; high × {rising/slowed}; not-cheap-or-high × {falling/rising/flat}). Descriptive only, no numbers restated from elsewhere on the page, no direction prediction — replaces the Groq-generated blurb (row 3, retired). | New 2026-08-10 (#765) |
