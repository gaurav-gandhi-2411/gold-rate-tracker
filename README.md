...
A free, **₹0/month** gold-price tracker for Indian retail buyers (22K), built to answer one question honestly: *is today a good time to buy?* No server, no database, no paid API — GitHub Actions scrapes and models the price on a 3h cron (see [docs/RUNBOOK.md](docs/RUNBOOK.md) for why "3h" is the design target, not the delivered cadence), GitHub Pages serves a static PWA that reads the committed JSON. Actual observed cadence, since GitHub's own scheduler has been dropping/delaying scheduled triggers since 2026-08-27, is a median of <!--METRIC:data/cadence_metrics.json#median_gap_hours:num1|n=n|asof=as_of-->4.6 (n=38, as of 2026-09-10)<!--/METRIC--> between successful data commits over the trailing week — worst case recently ~<!--METRIC:data/cadence_metrics.json#p90_gap_hours:num1-->6.7<!--/METRIC-->h. The whole stack costs nothing to run and has run unattended since 2026-05.
...
- **IBJA-primary, Tanishq-enrichment ([ADR 025](docs/adr/025-ibja-primary-source-decision.md)).** IBJA (India's national bullion-association benchmark) isn't Cloudflare-protected and reliably publishes a daily reading. Tanishq's site still blocks the plain-HTTP request path outright, but the self-hosted Playwright scraper ([docs/RUNBOOK.md](docs/RUNBOOK.md)) succeeds <!--METRIC:data/tanishq_scrape_success_rate.json#success_rate:pct1|n=n|asof=generated_at_utc-->100.0% (n=22, as of 2026-09-10)<!--/METRIC--> of the time over the last 7 days — IBJA stays primary regardless (Tanishq is opportunistic enrichment even when reachable, not a hard dependency). The displayed price defaults to an IBJA-calibrated estimate (R²=<!--METRIC:data/calibration.json#r_squared:num2|n=n_observations|asof=fit_date-->0.96 (n=75, as of 2026-08-28)<!--/METRIC-->), whose displayed range's real-world coverage is re-scored weekly (not a one-time reading) at <!--METRIC:data/calibration_band_coverage.json#coverage:pct1|n=n|ci=wilson_ci_low,wilson_ci_high|asof=generated_at_utc|unresolved_if=resolvable_at_n-->68.9% (n=74, 95% CI [57.7%, 78.3%], as of 2026-09-06)<!--/METRIC--> against an 80% nominal target, and upgrades to a directly-confirmed Tanishq reading only when that scrape succeeds within the last 8h — never the reverse. On IBJA's non-publishing days (weekends, holidays) the page carries forward the last published close, clearly dated. The user never sees a dead price.
