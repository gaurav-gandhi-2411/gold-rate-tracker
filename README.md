...
A free, **₹0/month** gold-price tracker for Indian retail buyers (22K), built to answer one question honestly: *is today a good time to buy?* No server, no database, no paid API — GitHub Actions scrapes and models the price on a 3h cron, GitHub Pages serves a static PWA that reads the committed JSON. Actual observed cadence, since GitHub's own scheduler has been dropping a share of scheduled triggers since 2026-08-27 (see [docs/RUNBOOK.md](docs/RUNBOOK.md)), is a median of <!--METRIC:data/cadence_metrics.json#median_gap_hours:num1|n=n|asof=as_of-->4.9 (n=36, as of 2026-09-21)<!--/METRIC--> between successful data commits over the trailing week. The whole stack costs nothing to run and has run unattended since 2026-05.
