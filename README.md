...
A free, **₹0/month** gold-price tracker for Indian retail buyers (22K), built to answer one question honestly: *is today a good time to buy?* No server, no database, no paid API — GitHub Actions scrapes and models the price on a 3h cron, GitHub Pages serves a static PWA that reads the committed JSON. Actual observed cadence, since GitHub's own scheduler has been dropping a share of scheduled triggers since 2026-08-27 (see [docs/RUNBOOK.md](docs/RUNBOOK.md)), is a median of <!--METRIC:data/cadence_metrics.json#median_gap_hours:num1|n=n|asof=as_of-->4.7 (n=41, as of 2026-09-23)<!--/METRIC--> between successful data commits over the trailing week. The whole stack costs nothing to run and has run unattended since 2026-05.
...
| Can any model call tomorrow's direction? | **No, at either horizon we test.** h=1: <!--METRIC:data/direction_baseline.json#horizons.h1.logistic_metrics.accuracy:pct1|n=horizons.h1.n_test_folds|asof=generated_at_utc-->48.5% (n=163, as of 2026-09-23)<!--/METRIC--> accuracy vs. a <!--METRIC:data/direction_baseline.json#horizons.h1.logistic_metrics.always_up_accuracy:pct1-->50.9%<!--/METRIC--> base rate (worse than guessing "up"). h=2: <!--METRIC:data/direction_baseline.json#horizons.h2.logistic_metrics.accuracy:pct1|n=horizons.h2.n_test_folds|asof=generated_at_utc-->61.5% (n=161, as of 2026-09-23)<!--/METRIC--> vs. a <!--METRIC:data/direction_baseline.json#horizons.h2.logistic_metrics.always_up_accuracy:pct1-->59.0%<!--/METRIC--> base rate — better-looking, but not statistically significant (p=<!--METRIC:data/direction_baseline.json#horizons.h2.logistic_metrics.p_value:num2-->0.45<!--/METRIC-->). Neither ships. | [`docs/DIRECTION_SIGNAL_STATUS.md`](docs/DIRECTION_SIGNAL_STATUS.md), auto-updated weekly |
