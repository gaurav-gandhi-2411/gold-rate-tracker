...
def _require_torch():
    if not _TORCH_AVAILABLE:
        raise ImportError(
            "torch is required for N-BEATS training. " "Install with: pip install torch"
        )
...
class NBeatsStack(nn.Module):
    ...
    def __init__(self, n_blocks: int, lookback: int, hidden: int, theta: int, dropout: float):
        ...
        self.blocks = nn.ModuleList(
            [NBeatsBlock(lookback, hidden, theta, dropout) for _ in range(n_blocks)]
        )
...
class NBeatsNet(nn.Module):
    ...
    def __init__(
        ...
        self.stacks = nn.ModuleList(
            [NBeatsStack(n_blocks, lookback, hidden, theta, dropout) for _ in range(n_stacks)]
        )
