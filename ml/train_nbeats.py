...
from ml.nbeats import (
    BATCH_SIZE, LOOKBACK, LR, MAX_EPOCHS,
    N_BLOCKS, N_STACKS, HIDDEN, THETA, DROPOUT, PATIENCE,
    WEIGHT_DECAY, NBeatsNet, build_sequences,
)
...
def train(max_epochs: int = MAX_EPOCHS) -> tuple:
    ...
    model = NBeatsNet(
        lookback=LOOKBACK, n_stacks=N_STACKS, n_blocks=N_BLOCKS,
        hidden=HIDDEN, theta=THETA, dropout=DROPOUT,
        scale=global_std,
    ).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, patience=15, factor=0.5, min_lr=5e-6,
    )
    ...
    print(f"\nBest epoch: {best_epoch}  val_MAE=Rs.{best_val_mae:.1f}  "
          f"({elapsed:.1f}s)")
...
def main():
    ...
    meta["smoke_test_delta"] = (round(delta, 1)
                                if not __import__("math").isnan(delta) else None)
