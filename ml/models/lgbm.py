...
class LightGBMForecaster(BaseForecaster):
    ...
    def _make_lgbm(self, objective: str, alpha: float | None = None) -> Any:
        ...
        kwargs: dict[str, Any] = dict(
            ...
            min_child_samples=int(p.min_data_in_leaf)
            if p and hasattr(p, "min_data_in_leaf") and p.min_data_in_leaf is not None
            else 20,
            reg_lambda=float(p.lambda_l2)
            if p and hasattr(p, "lambda_l2") and p.lambda_l2 is not None
            else 0.0,
            random_state=42,
            verbose=-1,
        )
