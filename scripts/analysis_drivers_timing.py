...
def _old_headline(total: int, gold: int, inr: int) -> dict:
    """Pre-fix app.js headline (templates assumed the rupee's sign). `sign_error` marks a
    sentence whose direction words contradict a part's sign."""
    g, r, up = abs(gold), abs(inr), total >= 0
    if r >= g and r > 10:
        key = "driverUpInrDominant" if up else "driverDownInrDominant"
        # up: "mostly a weaker rupee (Rs r), plus a bit from global gold prices (Rs g)";
        # down: "mostly a stronger rupee (Rs r), with global gold roughly flat"
        err = (inr < 0 or gold < 0) if up else (inr > 0 or g > 10)
    elif g > 10:
        key = "driverUpGoldDominant" if up else "driverDownGoldDominant"
        # up: "mostly global gold prices (Rs g), plus a bit from the rupee (Rs r)"; down: "global
        # gold fell about Rs g, and the rupee added back Rs r" ("roughly flat" when |r| <= 10)
        err = (gold < 0 or inr < 0) if up else (gold > 0 or inr < -10)
    else:
        key = "driverUpMixed" if up else "driverDownMixed"
        err = False
    return {"key": key, "total": abs(total), "gold": g, "inr": r, "sign_error": err}


def _new_headline(total: int, gold: int, inr: int) -> dict | None:
    """Post-fix app.js driverHeadlineText: each part worded by its own sign."""
    if total == 0:
        return None
    g, r = abs(gold), abs(inr)
    if g <= 10 and r <= 10:
        key = "driverUpMixed" if total > 0 else "driverDownMixed"
        return {"key": key, "total": abs(total), "gold": g, "inr": r, "sign_error": False}
    gold_key = "GoldFlat" if g <= 10 else ("GoldAdded" if gold > 0 else "GoldTookOff")
    inr_key = "RupeeFlat" if r <= 10 else ("RupeeAdded" if inr > 0 else "RupeeTookOff")
    parts = [inr_key, gold_key] if r > g else [gold_key, inr_key]
    lead = "WeekUp" if total > 0 else "WeekDown"
    key = f"driverHeadline:{lead}+{parts[0]}+{parts[1]}"
    return {"key": key, "total": abs(total), "gold": g, "inr": r, "sign_error": False}


def displayed(ctx: dict, style: str) -> dict:
    """Port of app.js renderDriverContext (pre-fix headline if style == "old")."""
    ...
    if w7 and w7.get("attribution_valid") and all(isinstance(w7.get(k), float | int) for k in keys):
        ...
        inr_s = math.floor(w7["usdinr_contrib_rs_per_g"] + 0.5)
        gold_s = math.floor(w7["gold_usd_contrib_rs_per_g"] + 0.5)
        old_wording = _old_headline(total, gold_s, inr_s)
        headline = old_wording if style == "old" else _new_headline(total, gold_s, inr_s)
    ...
    if inr_moved or gold_moved:
        ...
        state = "driverAllFlat"
    # What the pre-fix templates would have said for these numbers (the wording bug)
    return {"hidden": False, "headline": headline, "state": state, "old_wording": old_wording}
...
def main() -> None:
            ...
            rows.append(
                {
                    ...
                    "old": {"ctx": old, "display": displayed(old, "old")},
                    "new": {"ctx": new, "display": displayed(new, "new")},
                }
            )
    ...
    part2["display"] = {
        ...
        "headline_sign_errors_old": sum(
            bool((d.get("headline") or {}).get("sign_error")) for d in d_old
        ),
        "headline_sign_errors_new_code_old_wording": [
            r["date"]
            for r in rows
            if (r["new"]["display"].get("old_wording") or {}).get("sign_error")
        ],
        "headline_sign_errors_new": sum(
            bool((d.get("headline") or {}).get("sign_error")) for d in d_new
        ),
    ...
    print("\n(old headline = pre-fix code AND pre-fix wording; * = wording contradicts a sign)")
    print("date | 7d share old/new | valid old/new | headline old -> new | state old -> new")
    for r in rows:
        ...
        print(
            ...
            f" | {h[0].get('key')}{'*' if h[0].get('sign_error') else ''} "
            f"{h[0].get('total')},{h[0].get('gold')},{h[0].get('inr')} -> "
